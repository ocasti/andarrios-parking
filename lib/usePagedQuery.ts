...
export interface PagedQueryOptions {
  table: string;
  select?: string;                                  // columnas
  search?: { columns: string[]; value: string };    // ilike OR
  filters?: Record<string, any>;                    // eq simples
  isNull?: string[];                                // columnas con null
  notNull?: string[];                               // columnas no-null
...
export function usePagedQuery<T = any>(opts: PagedQueryOptions, deps: any[] = []): PagedQueryResult<T> {
