...
import { formatDate, formatTime } from '@/src/domain/services/FormatterService';
...
export default function ReportesPage() {
  ...
  const expResidentes = () => run('Residentes', async () => {
    ...
    data.forEach((r) => rows.push([r.cod, r.torre, r.piso, r.placa, r.tipo, r.nombre, r.cel || '', formatDate(r.fecha_registro || r.created_at)]));
  ...
  const expHistorial = () => run('Historial', async () => {
    ...
    data.forEach((h) => rows.push([formatDate(h.salida), h.placa, h.tipo, h.cod, h.nombre || '', formatTime(h.entrada), formatTime(h.salida), (h.horas ?? 0).toFixed(2), h.base ?? 0, h.iva ?? 0, h.total ?? 0]));
  ...
  const expControl = () => run('Control de acceso', async () => {
    ...
    blq.forEach((x) => b.push([x.cod, x.motivo, formatDate(x.fecha_bloqueo), x.desbloqueado_at ? 'Desbloqueado' : 'Bloqueado']));
    const a: any[][] = [['Código apto', 'Placa', 'Tipo', 'Fecha aprobación', 'Estado']];
    placas.forEach((p) => a.push([p.cod, p.placa, p.tipo, formatDate(p.fecha_aprobacion), p.deleted_at ? 'Revocada' : 'Activa']));
  ...
  const expCierres = () => run('Cierres de caja', async () => {
    ...
    data.forEach((c) => rows.push([formatDate(c.fecha), c.cobros_vis, c.total_vis, c.cobros_mens, c.total_mens, c.total_iva, c.total]));
  ...
  const expMov = () => run('Movimientos completos', async () => {
    ...
    visitantes.forEach((v) => {
      ...
      rows.push([formatDate(v.entrada), formatTime(v.entrada), v.salida ? formatTime(v.salida) : '(en parqueadero)', tipoMov, v.tipo, v.placa, v.nombre || '', v.tel || '', v.cod, (v.horas ?? 0).toFixed(2), v.base ?? '—', v.iva ?? '—', v.total ?? '—']);
    });
    ...
    pagos.forEach((p) => r2.push([formatDate(p.fecha), formatTime(p.fecha), p.placa || '', p.nombre || '', p.cod || '', p.tipo || '', p.monto]));
    ...
    cierres.forEach((c) => r3.push([formatDate(c.fecha), c.cobros_vis, c.total_vis, c.cobros_mens, c.total_mens, c.total_iva, c.total]));
